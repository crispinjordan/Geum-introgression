#This script creates 2 files that are used by another script to genotype samples at a set of species-specific SNPs.
...
whitefile = open('/PATH_TO_DATA/Whitelist_M6_PE_CorrAltV135_Filtered_NoM.txt', 'wt')
#Open an output file that will contain all the information about a locus, to be used in a 'dict' in a following analysis:
dictfile = open('/PATH_TO_DATA/DictInfo_M6_PE_CorrAltV135_Filtered_NoM.txt', 'wt')
...
#Skip the first 3 lines (lines 0,1,2) of the file:
	if i<3:
		...
		if previous_line[BatchID]==present_line[BatchID] and previous_line[LocusID]==present_line[LocusID] and previous_line[Chr]==present_line[Chr] and previous_line[BP]==present_line[BP] and previous_line[Col]==present_line[Col] and previous_line[PopID] != present_line[PopID] and previous_line[PNuc] != present_line[QNuc] and previous_line[QNuc] =='-' and present_line[PNuc] =='-':
			...
			#We will only work with the first snp for a given LocusID, which, in this case, is now the Chr (contig), so that we only use the first species-diagnostic snp for each contig.
			#check whether we are dealing with a new locus - if so, do the following - NOW BASED ON HAVING A SINGLE MARKER PER CONTIG:
			if uniqLocusID != present_line[Chr]:
				#re-assign the locusID:
				uniqLocusID = present_line[Chr]
